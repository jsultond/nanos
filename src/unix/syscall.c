...
{
    sysreturn rv;
    if ((write && !file_is_writable(f)) || (!write && !file_is_readable(f))) {
        rv = -EBADF;
        goto out;
    }
...
static sysreturn sendfile(int out_fd, int in_fd, int *offset, bytes count)
{
    ...
    if (!file_is_readable(infile) || !file_is_writable(outfile))
        return -EBADF;
...
sysreturn access(const char *name, int mode)
{
    ...
    tuple m;
    int ret = resolve_cstring_follow(current->p->cwd, name, &m, 0);
    if (ret) {
        return set_syscall_return(current, ret);
    }
    if (mode == F_OK)
